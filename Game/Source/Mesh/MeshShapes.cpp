...
void Mesh::CreateCube(vec3 size)
{
    ...
    // front
    verts[0] = VertexFormat( vec3( -size.x/2, -size.y/2, -size.z/2 ), MyColor( 255,255,255,255 ), vec3( 0,0,0 ), vec2( 0,0 ) );
    verts[1] = VertexFormat( vec3( -size.x/2,  size.y/2, -size.z/2 ), MyColor( 255,255,255,255 ), vec3( 0,0,0 ), vec2( 0,1 ) );
    verts[2] = VertexFormat( vec3(  size.x/2,  size.y/2, -size.z/2 ), MyColor( 255,255,255,255 ), vec3( 0,0,0 ), vec2( 1,1 ) );
    verts[3] = VertexFormat( vec3(  size.x/2, -size.y/2, -size.z/2 ), MyColor( 255,255,255,255 ), vec3( 0,0,0 ), vec2( 1,0 ) );

    // back
    verts[4] = VertexFormat( vec3(  size.x/2, -size.y/2,  size.z/2 ), MyColor( 255,255,255,255 ), vec3( 0,0,0 ), vec2( 0,0 ) );
    verts[5] = VertexFormat( vec3(  size.x/2,  size.y/2,  size.z/2 ), MyColor( 255,255,255,255 ), vec3( 0,0,0 ), vec2( 0,1 ) );
    verts[6] = VertexFormat( vec3( -size.x/2,  size.y/2,  size.z/2 ), MyColor( 255,255,255,255 ), vec3( 0,0,0 ), vec2( 1,1 ) );
    verts[7] = VertexFormat( vec3( -size.x/2, -size.y/2,  size.z/2 ), MyColor( 255,255,255,255 ), vec3( 0,0,0 ), vec2( 1,0 ) );
    ...
    // left
    verts[8]  = VertexFormat( vec3( -size.x/2, -size.y/2,  size.z/2 ), MyColor( 255,255,255,255 ), vec3( 0,0,0 ), vec2( 0,0 ) );
    verts[9]  = VertexFormat( vec3( -size.x/2,  size.y/2,  size.z/2 ), MyColor( 255,255,255,255 ), vec3( 0,0,0 ), vec2( 0,1 ) );
    verts[10] = VertexFormat( vec3( -size.x/2,  size.y/2, -size.z/2 ), MyColor( 255,255,255,255 ), vec3( 0,0,0 ), vec2( 1,1 ) );
    verts[11] = VertexFormat( vec3( -size.x/2, -size.y/2, -size.z/2 ), MyColor( 255,255,255,255 ), vec3( 0,0,0 ), vec2( 1,0 ) );

    // right
    verts[12] = VertexFormat( vec3(  size.x/2, -size.y/2, -size.z/2 ), MyColor( 255,255,255,255 ), vec3( 0,0,0 ), vec2( 0,0 ) );
    verts[13] = VertexFormat( vec3(  size.x/2,  size.y/2, -size.z/2 ), MyColor( 255,255,255,255 ), vec3( 0,0,0 ), vec2( 0,1 ) );
    verts[14] = VertexFormat( vec3(  size.x/2,  size.y/2,  size.z/2 ), MyColor( 255,255,255,255 ), vec3( 0,0,0 ), vec2( 1,1 ) );
    verts[15] = VertexFormat( vec3(  size.x/2, -size.y/2,  size.z/2 ), MyColor( 255,255,255,255 ), vec3( 0,0,0 ), vec2( 1,0 ) );
    ...
    // top
    verts[16] = VertexFormat( vec3( -size.x/2,  size.y/2, -size.z/2 ), MyColor( 255,255,255,255 ), vec3( 0,0,0 ), vec2( 0,0 ) );
    verts[17] = VertexFormat( vec3( -size.x/2,  size.y/2,  size.z/2 ), MyColor( 255,255,255,255 ), vec3( 0,0,0 ), vec2( 0,1 ) );
    verts[18] = VertexFormat( vec3(  size.x/2,  size.y/2,  size.z/2 ), MyColor( 255,255,255,255 ), vec3( 0,0,0 ), vec2( 1,1 ) );
    verts[19] = VertexFormat( vec3(  size.x/2,  size.y/2, -size.z/2 ), MyColor( 255,255,255,255 ), vec3( 0,0,0 ), vec2( 1,0 ) );

    // bottom
    verts[20] = VertexFormat( vec3(  size.x/2, -size.y/2, -size.z/2 ), MyColor( 255,255,255,255 ), vec3( 0,0,0 ), vec2( 0,0 ) );
    verts[21] = VertexFormat( vec3(  size.x/2, -size.y/2,  size.z/2 ), MyColor( 255,255,255,255 ), vec3( 0,0,0 ), vec2( 0,1 ) );
    verts[22] = VertexFormat( vec3( -size.x/2, -size.y/2,  size.z/2 ), MyColor( 255,255,255,255 ), vec3( 0,0,0 ), vec2( 1,1 ) );
    verts[23] = VertexFormat( vec3( -size.x/2, -size.y/2, -size.z/2 ), MyColor( 255,255,255,255 ), vec3( 0,0,0 ), vec2( 1,0 ) );
    ...
    Init( verts, 4, indices, 6, GL_TRIANGLES, GL_STATIC_DRAW );
};
...
void Mesh::CreatePlane(vec2 worldSize, ivec2 vertexCount, vec2 pivotPoint, vec2 uvRepeat)
{
    ...
    for( int y = 0; y < vertexCount.y; y++ )
    {
        for( int x = 0; x < vertexCount.x; x++ )
        {
            ...
            verts[y * vertexCount.x + x].m_Color = MyColor(255, 255, 255, 255);
        }
    }
