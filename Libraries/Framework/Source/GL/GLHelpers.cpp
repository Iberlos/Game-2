...
#include "GLHelpers.h"
#include "../../Framework/Libraries/LodePNG/lodepng.h"
...
GLuint LoadTexture(const char* filename, unsigned int* outWidth, unsigned int* outHeight)
{
    ...
    long filesize;
    unsigned char* filebuffer = reinterpret_cast<unsigned char*>( LoadCompleteFile( filename, &filesize ) );
...
void Flip32BitImageVertically(unsigned char* buffer, unsigned int width, unsigned int height)
{
    ...
    
    {
        ...
        unsigned int* buffer32 = reinterpret_cast<unsigned int*>( buffer );
