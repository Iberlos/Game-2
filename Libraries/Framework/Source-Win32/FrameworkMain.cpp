...
void Framework::ResizeWindow(int width, int height)
{
    ...
    //Clip the cursor to our window space
    RECT rect;
...
bool Framework::CreateGLWindow(char* title, int width, int height, char colorbits, char zbits, char stencilbits, bool fullscreenflag)
{
    ...
    //Sign up for RAW input of the mouse
...
void Framework::KillGLWindow()
{
    ...
    //Always show the cursor after closing the window
    ShowCursor(true);
...
LRESULT CALLBACK Framework::WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_KEYDOWN:
        {
            ...
            {
                Event* pEvent = new InputEvent(InputDeviceType_Keyboard, InputState_Held, wParam, 0);
                pFramework->GetGameCore()->GetEventManager()->QueueEvent(pEvent);
            }
    ...
    case WM_MOUSEMOVE:
        {
            ...
            int screenMidX = static_cast<int>(pFramework->GetWindowWidth() * 0.5);
            int screenMidY = static_cast<int>(pFramework->GetWindowHeight() * 0.5);
    ...
    case WM_SIZE:
        ...
        //If we have some RAW input
        //Copied a lot from the Windows docs
    case WM_INPUT:
        {
            UINT dwSize;
            ...
            {
                //GetRawInputData didn't return correct size
            }
            ...
            {
                //Apparently we always get relative movement for the mouse
                /*if (raw->data.mouse.usFlags & MOUSE_MOVE_RELATIVE)
                {*/
                    int lastX = raw->data.mouse.lLastX;
                    int lastY = raw->data.mouse.lLastY;

                    Event* pEvent = new InputEvent(InputDeviceType_RAW_Mouse, InputState_Held, 0, vec2((float)lastX, (float)lastY));
                    pFramework->GetGameCore()->GetEventManager()->QueueEvent(pEvent);
                //}
